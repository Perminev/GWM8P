...
# Определяем класс "Causes_un", который является расширением (Cog) для бота.
class Causes_un(commands.Cog):
    # Инициализируем класс "Causes_un" и сохраняем объект бота в атрибуте "self.bot".
    def __init__(self, bot):
        self.bot = bot

    # Определяем метод "causes_un", который будет вызываться при выполнении команды "/causes_un".
    @commands.command(description = 'Причины')
    async def causes_un(self, ctx):
...
# Определяем функцию "setup", которая добавляет расширение "Causes_un" в бота.
def setup(bot):
    bot.add_cog(Causes_un(bot))